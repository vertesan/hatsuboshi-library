...
For players who already have more than 500 Memories in their possessions, sorting them out can be a tough challenge because 
...
Sadly, the in-game 「メモリー変換」 UI is a nonsense which only provides us a little part of information about contest realated
things at first level interface, resulting thousands of players repeatly long press the Memories to see details again and again 
which imho should be considered a totally disregard of humanity that is intolerable. 

> Honestly, QualiArts should fire their UX designer who made this shit.

Having wasted a great bunch of time pressing my lovely smartphone again and again, I realized if there is a tool which can visualize 
...
Usually you don't want to make such an array of that large objects from scratch, the simplest way is capturing the HTTPS traffic 
from the game but that is not the content of this readme. 

After selecting the file, Memories you possessed will show up on the screen then you can examine most of the useful information 
on the first level interface. Note to make it more convenient to use we didn't design pagination for them, this comes at a 
cost which greatly reduces the response speed of the page if you have hundreds of Memories because they are rendered conditionlessly 
at the same time, we suggest users to use character filter to examine one character at a time.
